# All blueprint folders will have a routes.py
...
# the prefix will be in the url
# in the pokeapi, this would be 'pokemon' in pokeapi.co/api/v2/pokemon/ditto
# You can change that prefix to 'move' to get to the moves... etc.
...
def get_drones(current_user_token):
    owner = current_user_token.token # remember that current_user_token is our current user, current_user is a reserved flask term
    drones = Drone.query.filter_by(user_token=owner).all()
    response = drones_schema.dump(drones)
    return jsonify(response)

# retrieve one specific drone Endpoint
...
def update_drone(current_user_token, id):
    ...
    # we don't need session.add() because it's already in there
    db.session.commit()
    response = drone_schema.dump(drone)
    return jsonify(response)
